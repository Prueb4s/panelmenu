...
(function (window) {
  ...
  function createButtonElement(order, phoneOverride) {
    ...
    if (orderId && wasPressedRecently(orderId)) {
      // Devolver null para que no se muestre
      return null;
    }
...
const handleLogin = async (role) => {
  console.log('[auth] handleLogin start, role=', role);
  ...
  if (!email || !password) {
    logError('Email y contraseña son obligatorios.');
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errMsg = errJson.error_description || errJson.error || JSON.stringify(errJson);
      } catch (_) {
        const txt = await response.text();
        errMsg = txt || errMsg;
      }
      throw new Error(errMsg || "Credenciales incorrectas.");
    }
    ...
    console.log('[auth] login success, user id=', currentUserId);
    initUserView(role);
...
const loadProducts = async () => {
  ...
  const searchTermEl = document.getElementById('product-search');
  ...
  (data || []).forEach(p => {
    ...
    html += `
      <tr class="hover:bg-gray-50">
        ...
        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">$${p.price || 0}</td>
        <td class="px-3 py-4 whitespace-nowrap text-sm ${p.stock > 0 ? 'text-green-600' : 'text-red-600' }">${p.stock || 0}</td>
...
const attachProductListeners = () => {
  ...
  if (addBtn) addBtn.onclick = () => showProductModal(null);
  const stockFilter = document.getElementById('stock-filter');
  if (stockFilter) stockFilter.onchange = loadProducts;
  const productSearch = document.getElementById('product-search');
  if (productSearch) productSearch.oninput = loadProducts;
};
...
const renderSizesEditorHTML = (sizes = []) => {
  ...
  let html = `
    <div class="pt-4 border-t">
      ...
      <div id="sizes-editor" class="mt-3 space-y-2">
  `;
...
const attachSizesEditorHandlers = () => {
  const addBtn = document.getElementById('add-size-btn');
  if (addBtn) {
    addBtn.onclick = (e) => {
      const container = document.getElementById('sizes-editor');
      if (!container) return;
      ...
      const rem = div.querySelector('.remove-size-btn');
      if (rem) rem.onclick = () => div.remove();
    };
  }
...
const showProductModal = (product) => {
  ...
  // parse sizes from product (could be array, object, or JSON string)
  let parsedSizes = [];
  try {
    const raw = product?.sizes;
    if (raw) {
      if (typeof raw === 'string') {
        parsedSizes = JSON.parse(raw);
  ...
  document.getElementById('modal-body').innerHTML = `
    <div class="space-y-4">
      <label class="block"><span class="text-gray-700">Nombre</span><input id="modal-name" value="${(product?.name || '').toString().replace(/"/g, '&quot;')}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2"></label>
      ...
      <div class="flex flex-wrap gap-4 pt-4 border-t items-center">
      ...
      ${renderSizesEditorHTML(parsedSizes)}
    </div>
  `;
  ...
  // Attach handlers for sizes editor (add/remove)
  attachSizesEditorHandlers();
...
const saveProduct = async () => {
  ...
  try {
    ...
    const category = document.getElementById('modal-category') ? document.getElementById('modal-category').value : '';
    ...
    // Leer sizes desde el editor (si existen)
    let sizesArr = [];
    const sizesContainer = document.getElementById('sizes-editor');
    if (sizesContainer) {
    ...
    const productData = {
      name: name,
      description: document.getElementById('modal-description') ? document.getElementById('modal-description').value : '',
      category: category,
      price: price,
      stock: stock,
      isOffer: document.getElementById('modal-isOffer') ? document.getElementById('modal-isOffer').checked : false,
      featured: document.getElementById('modal-featured') ? document.getElementById('modal-featured').checked : false,
      bestSeller: document.getElementById('modal-bestSeller') ? document.getElementById('modal-bestSeller').checked : false,
...
const generateOrderTableCaja = (data) => {
  ...
  (data || []).forEach(o => {
    html += `
      <tr class="hover:bg-gray-50">
        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">${o.id ? o.id.substring(0, 8) + '...' : ''}</td>
...
const showOrderDetailModal = (order) => {
  if (!order) {
    logError('Orden no disponible para mostrar detalle.');
    return;
  }

  document.getElementById('modal-title').textContent = `Detalle de Orden #${order.id ? order.id.substring(0, 8) : ''}`;

  let totalItemsCount = 0;
  let subtotalItemsPrice = 0;

  // Manejo robusto y display del campo sizes (JSONB) por item.
  let itemsHtml = (order.order_items || []).map(item => {
    const itemQuantity = item.qty || item.quantity || 0;
    const itemPriceRaw = item.price || 0;

    // Intentar obtener el objeto/array sizes del item
    let sizesRaw = item.sizes ?? item.size_options ?? item.sizeOption ?? null;
    ...
    // Detectar nombre de tamaño seleccionado
    let sizeName = '';
    let sizePriceOverride = null;
    const selectedCandidate = item.size || item.size_name || item.selectedSize || item.sizeSelected || item.size_option || item.selected_option;

    if (parsedSizes) {
      if (Array.isArray(parsedSizes)) {
        let found = null;
        if (selectedCandidate !== undefined && selectedCandidate !== null && selectedCandidate !== '') {
          if (!isNaN(Number(selectedCandidate))) {
            const idx = Number(selectedCandidate);
            found = parsedSizes[idx];
          } else {
            found = parsedSizes.find(s => String(s.name) === String(selectedCandidate) || String(s.id) === String(selectedCandidate));
          }
        }
        if (!found && parsedSizes.length === 1) found = parsedSizes[0];
        if (found) {
          sizeName = found.name || '';
          if (found.price !== undefined && found.price !== null) sizePriceOverride = Number(found.price);
        } else {
          sizeName = parsedSizes.map(s => s.name).filter(Boolean).join(', ');
        }
      } else if (typeof parsedSizes === 'object') {
        sizeName = parsedSizes.name || '';
        if (parsedSizes.price !== undefined && parsedSizes.price !== null) sizePriceOverride = Number(parsedSizes.price);
      }
    } else {
      if (item.size_name) sizeName = item.size_name;
      else if (item.size) sizeName = String(item.size);
    }

    // Determinar precio por ítem (prefiere override por size)
    let itemPrice = (itemPriceRaw !== undefined && itemPriceRaw !== null) ? Number(itemPriceRaw) : 0;
    if (sizePriceOverride !== null && !isNaN(sizePriceOverride)) {
      itemPrice = sizePriceOverride;
    }
    ...
    return `
      <li class="py-1 border-b border-gray-100">
        <div class="flex justify-between">
          <span class="text-gray-700 font-medium">(${itemQuantity}x) ${item.name || item.product_name || 'Ítem'}${sizeLabel}</span>
...
const printOrder = (order) => {
  ...
  printArea.innerHTML = `
    ...
    <div class="mb-2">--------------------------------</div>
    <div>Orden ID: ${order.id ? order.id.substring(0, 8) : ''}</div>
...
const renderOrdersCartera = (data) => {
  ...
  if (totalEl) totalEl.textContent = `Costo Total Vendido: $${totalVendido.toLocaleString('es-CO')}`;
};
...
const applyCarteraFilters = () => {
  ...
  const searchTerm = (document.getElementById('cartera-search') ? document.getElementById('cartera-search').value.toLowerCase() : '');
...
const isShiftOpen = () => !!localStorage.getItem('shift_start');
  ...
  try {
    ...
    (orders || []).forEach(o => {
      ...
      items.forEach(it => {
        ...
        if (!qty || qty <= 0) return;
        const pid = it.id || it.product_id || (it.product && it.product.id);
        const name = it.name || (it.product && it.product.name) || 'Sin nombre';
        if (!pid) return;
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('[app] DOMContentLoaded - inicializando listeners');

  document.querySelectorAll('.login-button').forEach(button => {
    button.onclick = () => {
      const role = button.getAttribute('data-role');
      console.log('[ui] login button clicked, role=', role);
      handleLogin(role);
    };
  });
  ...
  const applyFiltersBtn = document.getElementById('apply-cartera-filters');
  if (applyFiltersBtn) applyFiltersBtn.onclick = applyCarteraFilters;
  const exportBtn = document.getElementById('export-cartera-btn');
  if (exportBtn) exportBtn.onclick = exportCarteraToCSV;
  const closeModalBtn = document.getElementById('close-modal');
  if (closeModalBtn) closeModalBtn.onclick = () => setView('modal', false);
  ...
  // Forzamos que se muestre el login por defecto para evitar casos donde el script prevenga interacción
  setView('login-view', true);
  setView('header-nav', false);
  setView('content-views', false);
});

// Exponer funciones útiles al window (si existen)
try {
